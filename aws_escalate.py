...
def main(args):
    ...
    if args.profile is None:
        ...
        aws_session_token = boto3.session.Session(profile_name=args.profile)
        client = aws_session_token.client('iam')

    # Since we may be calling with role credentials, try to get the username of the current user.
    # If that username is an exception, then let's just grab all users.
    try:
        current_user = client.get_user()['User']
    except: # disable
        current_user = None

    # Now get all users if we explicitly say to, or if the current permissions come from a non-user
    if args.all_users is True or current_user is None:
        ...
        # if we get here, then we're a user, 
        current_user = {
        ...
        raise Exception("Insufficient clarity on which users to scan for permissions issues.")
    ...
    all_perms = [
        ...
        'iam:CreatePolicyVersion', 'iam:CreateLoginProfile', 'iam:PassRole',
        'iam:PutGroupPolicy', 'iam:PutRolePolicy', 'iam:PutUserPolicy',
        'iam:SetDefaultPolicyVersion', 'iam:UpdateAssumeRolePolicy',
        'iam:UpdateLoginProfile', 'sts:AssumeRole', 'ec2:RunInstances',
        'lambda:CreateEventSourceMapping', 'lambda:CreateFunction',
    ...
    for user in users:
                        ...
                        for user_perm in user['Permissions'][effect].keys():
                            if '*' in user_perm:
                                ...
                                if pattern.search(perm) is not None:
                                    checked_perms[effect][perm] = user[
                                        'Permissions'][effect][user_perm]
        ...
        for method in escalation_methods:
            ...
            for perm in escalation_methods[method]:
                if perm not in checked_perms[
                        'Allow']:  # If this permission isn't Allowed, then this method won't work
    ...
    file.close()
    print(
        'Privilege escalation check completed. Results stored to ./all_user_privesc_scan_results_{}.csv'.
        format(now))
...
def parse_document(document, user):
    ...
    for statement in document['Statement']:
        if statement['Effect'] == 'Allow':
            if 'Action' in statement and type(
                    statement['Action']
            ) is list:  # Check if the action is a single action (str) or multiple (list)
                ...
                user['Permissions']['Allow'][statement['Action']] = list(
                    set(user['Permissions']['Allow'][statement[
                        'Action']]))  # Remove duplicate resources
            if 'NotAction' in statement and type(
                    statement['NotAction']
            ) is list:  # NotAction is reverse, so allowing a NotAction is denying that action basically
                ...
                for not_action in statement['NotAction']:
                    ...
                    user['Permissions']['Deny'][not_action] = list(
                        set(user['Permissions']['Deny'][
                            not_action]))  # Remove duplicate resources
            elif 'NotAction' in statement and type(
                    statement['NotAction']) is str:
                if statement['NotAction'] in user['Permissions']['Deny']:
                    if type(statement['Resource']) is list:
                        user['Permissions']['Deny'][statement[
                            'NotAction']] += statement['Resource']
                    else:
                        user['Permissions']['Deny'][statement[
                            'NotAction']].append(statement['Resource'])
                else:
                    if type(statement['Resource']) is list:
                        user['Permissions']['Deny'][statement[
                            'NotAction']] = statement['Resource']
                ...
                user['Permissions']['Deny'][statement['NotAction']] = list(
                    set(user['Permissions']['Deny'][statement[
                        'NotAction']]))  # Remove duplicate resources
            ...
            if 'Action' in statement and type(statement['Action']) is list:
                ...
                for action in statement['Action']:
                    if action in user['Permissions']['Deny']:
                        if type(statement['Resource']) is list:
                            user['Permissions']['Deny'][action] += statement[
                                'Resource']
                        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=False,
        action='store_true',
        help='Run this module against every user in the account.')
    parser.add_argument(
        '--user-name',
        required=False,
        default=None,
        help=
        'A single username of a user to run this module against. By default, the user to which the active AWS keys belong to will be used.'
    )
    parser.add_argument(
        '--profile',
        required=False,
        default=None,
        help='Choose a specific profile to assess.')
